function [C,c,e_k,y_te,mse_te_k] = MH_NICE_M_QKLMS(X,T,X_te,T_te,TD,N_tr,N_te,d_c,d_q,lr_k)
...
for n=2:N_tr
   ...
   if(d_min_q < d_q)
       % M-QKLMS
       alpha(n) = alpha(n) + lr_k*e_k(n)'*(exp(-sum((C{1,d_arg_q}*ones(size(C{1,d_arg_q},2),n-1)-X(:,1:n-1)).^2)))';
